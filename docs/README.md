...
Le centre d’alertes Premium lit uniquement les instantanés déjà sauvegardés : variation d’ImmoValue avec confiance suffisante, variation du rôle municipal et perte de flux dans le test « Taux +1 point ». L’utilisateur active le suivi dossier par dossier dans **Mes propriétés**, ou juste après la première sauvegarde; il peut aussi rouvrir le dossier concerné depuis une alerte. Le choix est local, isolé par compte et ne stocke qu’une empreinte du nom normalisé du dossier. Il ne déclenche aucun appel externe ni courriel pendant la bêta.
