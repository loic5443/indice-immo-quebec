...
INDEX = (
    ...
    "99999,Ville-exemple,https://mamh.gouv.qc.ca/role/RM99999.xml,2025-12-19\n"
).encode()
XML = (
    b'\xef\xbb\xbf<?xml version="1.0"?><RL><VERSION>2.9</VERSION><RLM01A>99999</RLM01A>'
...
class AddressFormUiTests(unittest.TestCase):
    def setUp(self):
        ...
        import_territory(1, self.db, "99999", lambda _: XML)
        self.app_source = (
            "from pathlib import Path\n"
    ...
    def test_street_only_submission_keeps_city_postal_and_consent_canonical(self):
        ...
        self.assertTrue(app.checkbox(key="address_form_consent").value)
        self.assertEqual(list(app.error), [])
        self.assertTrue(any("Ville-exemple (territoire 99999)" in item.value for item in app.success))
